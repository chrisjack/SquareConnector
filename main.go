...
func startCLI(config AppConfig, stateManager *core.StateManager, bluetoothManager *core.BluetoothManager, launchMonitor *core.LaunchMonitor) {
	...
	// Setup OpenConnect integration
	log.Println("Starting OpenConnect integration")
	ocIntegration := openconnect.GetInstance(stateManager, launchMonitor, config.OpenConnectIP, config.OpenConnectPort)
	ocIntegration.EnableAutoReconnect()
	ocIntegration.Start()
...
func startWebServer(config AppConfig, stateManager *core.StateManager, bluetoothManager *core.BluetoothManager, launchMonitor *core.LaunchMonitor) {
	...
	// Create web server
	server := web.NewServer(stateManager, bluetoothManager, launchMonitor, cameraManager, config.GSProIP, config.GSProPort, config.InfiniteTeesIP, config.InfiniteTeesPort, config.OpenConnectIP, config.OpenConnectPort, config.EnableExternalCamera)
	...
	if settings.OpenConnectAutoConnect {
		log.Printf("Auto-connecting to OpenConnect at %s:%d", settings.OpenConnectIP, settings.OpenConnectPort)
		ocIntegration := server.GetOpenConnectIntegration()
		ocIntegration.EnableAutoReconnect()
		ocIntegration.Start()
		go ocIntegration.Connect(settings.OpenConnectIP, settings.OpenConnectPort)
	}
...
func main() {
	...
	ocIP := flag.String("oc-ip", "127.0.0.1", "IP address of OpenConnect simulator server")
	ocPort := flag.Int("oc-port", 922, "Port of OpenConnect simulator server")
	...
	openConnectIP := *ocIP
	openConnectPort := *ocPort
	if openConnectIP == "127.0.0.1" && savedSettings.OpenConnectIP != "" {
		openConnectIP = savedSettings.OpenConnectIP
	}
	if openConnectPort == 922 && savedSettings.OpenConnectPort != 0 {
		openConnectPort = savedSettings.OpenConnectPort
	}
	...
	config := AppConfig{
		...
		OpenConnectIP:        openConnectIP,
		OpenConnectPort:      openConnectPort,
