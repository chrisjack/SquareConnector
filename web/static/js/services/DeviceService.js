...
export class DeviceService {
    ...
    async armBallDetection() {
        try {
            const response = await this.api.post('/api/device/practice', { enabled: true });
            if (response.ok) {
                this.eventBus.emit('device:armed');
                return { success: true };
            } else {
                throw new Error(`Failed to arm ball detection: ${response.statusText}`);
            }
        } catch (error) {
            this.eventBus.emit('device:error', error.message);
            return { success: false, error: error.message };
