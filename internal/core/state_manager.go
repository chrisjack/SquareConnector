...
type StateManager struct {
	...
	callbacks struct {
		...
		OpenConnectStatus   []StateCallback[OpenConnectConnectionStatus]
		OpenConnectError    []StateCallback[error]
...
func (sm *StateManager) initialize() {
	...
	sm.state = AppState{
		...
		OpenConnectStatus:    OpenConnectStatusDisconnected,
...
// GetOpenConnectStatus returns the OpenConnect connection status
func (sm *StateManager) GetOpenConnectStatus() OpenConnectConnectionStatus {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
	return sm.state.OpenConnectStatus
}

// SetOpenConnectStatus sets the OpenConnect connection status
func (sm *StateManager) SetOpenConnectStatus(value OpenConnectConnectionStatus) {
	sm.mu.Lock()
	oldValue := sm.state.OpenConnectStatus
	sm.state.OpenConnectStatus = value
	callbacks := sm.callbacks.OpenConnectStatus
	sm.mu.Unlock()
...
// GetOpenConnectError returns the OpenConnect error
func (sm *StateManager) GetOpenConnectError() error {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
	return sm.state.OpenConnectError
}

// SetOpenConnectError sets the OpenConnect error
func (sm *StateManager) SetOpenConnectError(value error) {
	sm.mu.Lock()
	oldValue := sm.state.OpenConnectError
	sm.state.OpenConnectError = value
	callbacks := sm.callbacks.OpenConnectError
	sm.mu.Unlock()
...
// RegisterOpenConnectStatusCallback registers a callback for OpenConnect status changes
func (sm *StateManager) RegisterOpenConnectStatusCallback(callback StateCallback[OpenConnectConnectionStatus]) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	sm.callbacks.OpenConnectStatus = append(sm.callbacks.OpenConnectStatus, callback)
}

// RegisterOpenConnectErrorCallback registers a callback for OpenConnect error changes
func (sm *StateManager) RegisterOpenConnectErrorCallback(callback StateCallback[error]) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	sm.callbacks.OpenConnectError = append(sm.callbacks.OpenConnectError, callback)
}
